...
public class PaymentService {
	...
	public PaymentResponse pay(Payment payment) {
		...
		String messageString = dao.payNow(payment);
		PaymentResponse response = new PaymentResponse();
		response.setStatus("success");
		response.setMessage(messageString);
		response.setTxtDate(new SimpleDateFormat("dd/mm/yyyy HH:mm:ss a").format(new Date()));
		return response;
	}
	
	public PaymentResponse getTranscation(String vendor) {
		PaymentResponse response = new PaymentResponse();
